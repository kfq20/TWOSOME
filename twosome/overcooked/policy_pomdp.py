...
class LLMAgent(nn.Module):
    def __init__(self, normalization_mode = 'token', load_path = None, load_8bit = False, task = 3):
        ...
        self.tokenizer = LlamaTokenizer.from_pretrained(self.base_model, padding_side='left')
    ...
    def get_action_and_value(self, obs, action=None, is_warmup=False, return_value = True):
        ...
        if self.cot:
            ...
            obs_inputs = self.tokenizer(prompt, return_tensors="pt", padding=True)
            obs_input_ids = obs_inputs["input_ids"].to(self.device)
            obs_attention_mask = obs_inputs["attention_mask"].to(self.device)
            ...
            with torch.no_grad():
                # obs_with_thoughts_tokens = self.actor(obs_input_ids, attention_mask=obs_attention_mask)
                obs_with_thoughts_tokens = self.llama.generate(**generate_input)
                obs_with_thoughts = self.tokenizer.batch_decode(obs_with_thoughts_tokens, skip_special_tokens=True, clean_up_tokenization_spaces=False)
                print(obs_with_thoughts[0])
                for i in range(len(obs_with_thoughts)):
                    obs_with_thoughts[i] = obs_with_thoughts[i].replace("What should you do first? Give me your reasoning process.", "Your thought is as following:")
                    obs_with_thoughts[i] += "\nSo you should first"
                # torch.cuda.empty_cache()
                # with open('thought.txt', 'w') as file:
                #     file.write(obs_with_thoughts[0])
            # need process obs_with_thoughts
            new_text_obs = [obs_with_thoughts[i] for i in range(len(text_obs))]
            sequence = []
            for p, ac in zip(new_text_obs, action_list):
        ...
        if self.task == 3:
            ...
            if cindex >= 0:
                ...
                if len(overlay["in_agent"]) == 0:
                    ...
                    action_list[5] = action_template.format("bowl", "second")
            if self.cot:
                sentences.append("To serve the dish of a bowl only containing chopped tomato and lettuce, what should you do first? Let's think step by step.")
            else:
            ...
            if cindex >= 0:
                ...
                if len(overlay["in_agent"]) == 0:
                    ...
                    action_list[2] = action_template.format("bowl") 
            if self.cot:
                sentences.append("To serve the dish of a bowl only containing chopped tomato, here are the available actions: ")
                for action in action_list:
                    sentences.append(f"{action},")
                sentences.append("What should you do first? Give me your reasoning process.")
                print("sentense", sentences)
            else:
